...
##"anneal.arff", "audiology.arff", "autos.arff", "credit-a.arff", \
...
for fname in ["anneal.arff", "audiology.arff", "autos.arff", "credit-a.arff", \
    ...
    if categorical.shape[0] == 0:
        ...
        for i in range(len(categorical[0])):
            print(i)
            a = LabelEncoder().fit_transform(list(categorical[:, i]))
            b = OneHotEncoder(n_values = 'auto',
                              categorical_features = 'all',
                              dtype = np.int32,
                              sparse = False,
                              handle_unknown = 'error').fit_transform(a.reshape(-1, 1))
        ...
        for i in range(2, len(L)):
            final_result = np.hstack((final_result, L[i]))


        categorical = final_result
    ...
    aa = LabelEncoder().fit_transform(Y)
    Y = np.array([aa], dtype = np.int)
