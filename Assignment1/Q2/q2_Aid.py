...
def load_data(path):
    ...
    for i in range(len(data)):
        for j in range(len(data[0])):
            if data[i][j] == None:
                data[i][j] = len(dataset['attributes'][j][1])
...
def features_encoders(features, categorical_features = 'all'):
    ...
    for i in range(n_features):
        ...
        label_encoders[i].fit(list(feature_i))
        X_int[:, i] = label_encoders[i].transform(list(feature_i))
...
for path in paths:
    ...
    dataset, data, masks = load_data(path)
